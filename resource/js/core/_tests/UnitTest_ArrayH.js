(function() {
	...
	describe('ArrayH', {
		...
		'map': function() {
			value_of(ArrayH.map([1, 2, 3], function(o, i, arr) {
				return o * 5;
			})[1]).should_be(10);
		},
		'forEach': function() {
			ArrayH.forEach([1, 2, 3], function(o, i, arr) {
				value_of(o).log("item");
		...
		'some': function() {
			var arr = [-2, -1, 0, 1, 2];
			value_of(ArrayH.some(arr, function(o, i, a) {
				return o > 0;
			})).should_be(true);
			value_of(ArrayH.some(arr, function(o, i, a) {
				return o > 2;
		...
		'every': function() {
			var arr = [-2, -1, 0, 1, 2];
			value_of(ArrayH.every(arr, function(o, i, a) {
				return o > -3;
			})).should_be(true);
			value_of(ArrayH.every(arr, function(o, i, a) {
				return o > 0;
		...
		'reduce': function() {
			var arr = [-2, -1, 0, 1, 2, 3];
			value_of(ArrayH.reduce(arr, function(a, b) {
				value_of([a, b]).log();
				return a + b;
			})).should_be(3);
			value_of(ArrayH.reduce(arr, function(a, b) {
		...
		'reduceRight': function() {
			var arr = [1, 2, 3];
			value_of(ArrayH.reduceRight(arr, function(a, b) {
				return a / b;
			})).should_be(1.5);
			value_of(ArrayH.reduceRight(arr, function(a, b) {
				return a / b;
